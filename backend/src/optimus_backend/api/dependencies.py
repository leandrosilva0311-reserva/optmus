...
from optimus_backend.infrastructure.persistence.postgres_api_keys import PostgresApiKeyRepository
...
def get_tool_executor() -> ToolExecutor:
    ...
    tools = {
        ...
        "diff_analysis": DiffAnalysisTool(),
        "log_analysis": LogAnalysisTool(),
        "config_inspection": ConfigInspectionTool(),
...
@lru_cache(maxsize=1)
def get_api_key_repository() -> object:
    if config.app_env == "test":
        return InMemoryApiKeyRepository()
    return PostgresApiKeyRepository(config.database_url)


@lru_cache(maxsize=1)
def get_api_key_use_case() -> ApiKeyUseCase:
    return ApiKeyUseCase(repository=get_api_key_repository(), pepper=config.secret_key)
...
def get_git_local_source_connector_factory() -> object:
    return GitLocalSourceConnector


def _try_authenticate_user(x_session_id: str) -> dict[str, str] | None:
    if not x_session_id:
        return None
    _, _, _, _, sessions, users, _, _, _ = get_repositories()
...
def get_current_user(
    ...
    user = _try_authenticate_user(x_session_id)
    if user:
        return user

    provided_key = x_api_key or _extract_bearer(authorization)
    if provided_key:
        record = get_api_key_use_case().authenticate(provided_key)
        if record:
            return {
                "auth_type": "api_key",
